...
```bash
docker run --name leadpydb -e MYSQL_ROOT_PASSWORD=strongpassword -e MYSQL_DATABASE=leadpy -e MYSQL_USER=leadpy -e MYSQL_PASSWORD=leadpy --restart always -p 3306:3306 -d mariadb:latest --character-set-server=utf8mb4 --collation-server=utf8mb4_bin
...
```bash
git clone https://github.com/leadpy/Multi360Digital
```
...
```bash
cp leadpy/backend/.env.example leadpy/backend/.env
nano leadpy/backend/.env
```
...
pm2 start dist/server.js --name leadpy-backend
```
...
```bash
pm2 start server.js --name leadpy-frontend
pm2 save
```
...
├─────┼─────────────────────────┼─────────────┼─────────┼─────────┼──────────┼────────┼──────┼───────────┼──────────┼──────────┼──────────┼──────────┤
│ 1   │ leadpy-frontend      │ default     │ 0.1.0   │ fork    │ 179249   │ 12D    │ 0    │ online    │ 0.3%     │ 50.2mb   │ deploy   │ disabled │
│ 6   │ leadpy-backend       │ default     │ 1.0.0   │ fork    │ 179253   │ 12D    │ 15   │ online    │ 0.3%     │ 118.5mb  │ deploy   │ disabled │
└─────┴─────────────────────────┴─────────────┴─────────┴─────────┴──────────┴────────┴──────┴───────────┴──────────┴──────────┴──────────┴──────────┘
...
```bash
sudo nano /etc/nginx/sites-available/leadpy-frontend
```
...
```bash
sudo cp /etc/nginx/sites-available/leadpy-frontend /etc/nginx/sites-available/leadpy-backend
sudo nano /etc/nginx/sites-available/leadpy-backend
```
...
```bash
sudo ln -s /etc/nginx/sites-available/leadpy-frontend /etc/nginx/sites-enabled
sudo ln -s /etc/nginx/sites-available/leadpy-backend /etc/nginx/sites-enabled
```
...
To run leadpy using docker you must perform the following steps:
...
User: admin@mantle.com
Password: admin

## Upgrading

leadpy is a working in progress and we are adding new features frequently. To update your old installation and get all the new features, you can use a bash script like this:
...
```bash
#!/bin/bash
echo "Updating leadpy, please wait."
